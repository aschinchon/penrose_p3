...
CreateKite <- function() {
  ...
  x <- c(0, c, 0)
  y <- c(0, 0, 0)
  xend <-  c(c, b*cos(A*pi/180), b*cos(A*pi/180))
  yend <-  c(0, b*sin(A*pi/180), b*sin(A*pi/180))
  st <- c(2, 3, 1)
...
# The sides of triangles 123 are labeled with 1, 2 and 3 and are divided into two
# other triangles using this function.
# This function receives a triangle of type 123 and divides it. It returns two
# other subtriangles: one of type 123 and another one of type 124
DivideTriangle123 <- function(df) {
  # Add longitude of sides to df
  df %>% 
    mutate(lng = sqrt((xend-x)^2+(yend-y)^2)) -> df
  
  # Longitude of the new side 1
  df %>% 
    filter(st == 3) %>% 
    select(lng) %>% as.numeric -> d1
  
  # Gradient of the new side 2
  df %>% 
    filter(st == 1) %>%
    transmute(gx = xend - x,
              gy = yend - y) %>% as.numeric() -> g1
  
  # Inner point of side 1 to divide the triangle
  df %>% filter(st == 1) %>% select(x, y) %>% 
    as.numeric() + d1 * g1/sqrt(sum(g1^2)) -> p1
  

  # df1: subtriangle (type 123)
        c(df %>% filter(st == 3) %>% select(x, y) %>% as.numeric(), 
          p1) %>% 
  ...
  # df2: subtriangle (type 124)
        c(df %>% filter(st == 3) %>% select(x, y) %>% as.numeric(), 
          p1) %>% 
  rbind(c(p1, 
          df %>% filter(st == 1) %>% select(x, y) %>% as.numeric())) %>% 
  rbind(c(df %>% filter(st == 3) %>% select(x, y) %>% as.numeric(), 
          df %>% filter(st == 2) %>% select(x, y) %>% as.numeric())) -> df2
  
  colnames(df2) <- c("x", "y", "xend", "yend")
  as_tibble(df2) %>% mutate(st = c(1,2,4)) -> df2
...
# and Triangle124 following the specs of this site:
# https://tartarus.org/~simon/20110412-penrose/penrose.xhtml
# The sides of triangles 124 are labeled with 1, 2 and 4 and are divided into three
# other triangles using this function.
# This function receives a triangle of type 124 and divides it. It returns two
# other subtriangles: two of type 124 and another one of type 123
DivideTriangle124 <- function(df) {
  # Add longitude  
  df %>% 
    mutate(lng = sqrt((xend-x)^2+(yend-y)^2)) -> df
  
  # Longitude of new sides 4
  df %>% 
    filter(st == 2) %>% 
    select(lng) %>% as.numeric -> d1
  ...
  # Longitude of new sides 1
  sqrt(sum((p1 - (df %>% filter(st == 2) %>% select(x, y) %>% as.numeric())) ^ 2)) -> d2
  ...
  df %>% 
    filter(st == 1) %>%
    select(x, y) %>% 
    as.numeric() + d2 * g2/sqrt(sum(g2^2)) -> p2
  
# df1 subtriangle (type 124)
          c(df %>% filter(st == 4) %>% select(xend, yend) %>% as.numeric(),
            p1) %>% 
    rbind(c(p1, 
            df %>% filter(st == 2) %>% select(x, y) %>% as.numeric())) %>% 
      rbind(c(df %>% filter(st == 4) %>% select(xend, yend) %>% as.numeric(), 
            df %>% filter(st == 2) %>% select(x, y) %>% as.numeric())) -> df1
  colnames(df1) <- c("x", "y", "xend", "yend")
  as_tibble(df1) %>% mutate(st = c(1,2,4)) -> df1
  ...
  rbind(c(p2, 
          df %>% filter(st == 1) %>% select(x, y) %>% as.numeric())) %>% 
  rbind(c(p1, 
          df %>% filter(st == 1) %>% select(x, y) %>% as.numeric())) -> df2
        
  colnames(df2) <- c("x", "y", "xend", "yend")
  as_tibble(df2) %>% mutate(st = c(1,2,4)) -> df2
  
  # df3 subtriangle (type 123)
  c(p1,
    p2) %>% 
    rbind(c(p1, 
            df %>% filter(st == 2) %>% select(x, y) %>% as.numeric())) %>% 
  ...
  as_tibble(df3) %>% mutate(st = 1:3) -> df3
